...
const GroupCard = ({group}) => {
    return (
        <motion.div  className="p-6 grid md:grid-cols-2 lg:grid-cols-3 gap-6"
        initial={{ opacity: 0, y: 40 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.9, ease: 'easeOut' }}>
     
        <div
          className="w-80 h-[460px] bg-white border border-gray-300 rounded-lg shadow-md flex flex-col items-center justify-start relative overflow-hidden font-sans transition-transform duration-300 hover:scale-105 border-2 border-teal-800"
        >
          {/* Top red bars */}
          ...
          <div className="absolute bottom-5 w-full px-3 pt-8 flex flex-col justify-end items-end">
            ...
            <Link to={`/groups/${group._id}`} className="bg-teal-800 w-[50%] rounded-full text-white py-2 px-3  flex justify-center items-center gap-2">See More <FaArrowRight></FaArrowRight></Link>
          </div>
        </div>
